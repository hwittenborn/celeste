    ...
    fn get_sections(
        ...
        submit_button.connect_clicked(glib::clone!(@weak window, @weak server_name => move |_| {
            ...
            let mut process = Command::new("rclone")
                .args(["authorize", "pcloud", DEFAULT_CLIENT_ID, DEFAULT_CLIENT_SECRET])
                .stdout(Stdio::piped())
                .stderr(Stdio::piped())
                .spawn()
                .unwrap();
            ...
            let dialog = MessageDialog::builder()
                .title(&libceleste::get_title!("Authentication to pCloud"))
                .heading("Authenticating to pCloud...")
            ...
            loop {
                ...
                // Otherwise if the command has finished, check if it returned a good exit code and then return it.
                    ...
                    if !exit_status.success() {
                        ...
                        gtk_util::show_codeblock_error("Authentication Error", "There was an issue authenticating to pCloud", &stderr_string);
                        window.set_sensitive(true);
                        break;
                    } else {
                        ...
                        sender.send(Some(ServerType::PCloud(PCloudConfig {
